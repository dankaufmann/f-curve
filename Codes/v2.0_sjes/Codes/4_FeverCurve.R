...
#-------------------------------------------------------------------------------------
# V 2.0
# - We use only a 3 day MA for the news because we drop outliers in the data part
...
#                   Tecon)
whichInd   <- c(1, 2, 3, 4, 5, 7, 11, 14, 15, 16, 17, 18)
whichNews  <- c(7, 11)
whichFin   <- c(1, 2, 3, 4, 5, 14, 15, 16, 17, 18)
...
# Financial and news subindices
DataCor <- as.data.frame(ts_c(fc_news, fc_fin))
Correl  <- cor(DataCor, use = "na.or.complete")[1, 2]
p <- ts_ggplot(
 # `f-curve`             = fc,
  ` f-curve news`      = fc_news,
  `f-curve financial` = fc_fin
)
p <- ggLayout(p)
p <- ggColor2(p)
p <- addCorr(p, Correl, "2015-01-01", 10)
#p <- addLines(p, myLines, myLabels, -18)
p
ggsave(filename = "../Results/MainNewsFin.pdf", width = figwidth, height = figheight)
