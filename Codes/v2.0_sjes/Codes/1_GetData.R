...
# - Tagi archive not available anymore. Multiple solutions included
#-------------------------------------------------------------------------------------
...
# TODO:  To be deleted later on
# TODO: Integrate in automatic update
#df_all_ch_all & df_all_int_all: FUW + NZZ + TA Archiv + TA Web 
#df_all_ch_comb & df_all_int_comb: FUW + NZZ + kombinierte Variante TA
#df_all_ch_web & df_all_int_web: FUW + NZZ + TA Web
#df_all_ch & df_all_int: Wie bisher (FUW + NZZ + TA Archiv)
#df_fuw_nzz_ch & df_fuw_nzz_int: FUW + NZZ 
load(file = "../News/RevisionTaToWeb/toTestTa.RData" )

#News.CH <- df_all_ch_new %>%
#  select("time", "mean") %>%
#  ts_xts() %>%
...
News.all.CH <- df_all_ch_all %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)

News.comb.CH <- df_all_ch_comb %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)

News.ex.CH <- df_fuw_nzz_ch  %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)
...
News.all.FOR <- df_all_int_all %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)

News.comb.FOR <- df_all_int_comb %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)

News.ex.FOR <- df_fuw_nzz_int %>%
  select("time", "mean") %>%
  ts_xts() %>%
  ts_span(start = "2000-01-01", end = endDate)

# Clear outliers
for (n in c("News.CH", "News.all.CH", "News.comb.CH", "News.ex.CH", "News.FOR", "News.all.FOR", "News.comb.FOR", "News.ex.FOR")) {
  myVar = get(n)
...
Indicators <- ts_c(TS.CH, RP.CH, RPShort.CH, VIX.CH, IRDIFF.CH, News.CH, News.all.CH, News.comb.CH, News.ex.CH,
                   News.FOR, News.all.FOR, News.comb.FOR, News.ex.FOR, RP.FOR, RPShort.FOR, TS.US, VIX.US, TS.EUR,
                   Tecon, SMI)
